...
public interface ScoreRepository extends CrudRepository<ScoreCard, Long> {
    /**
     * Gets the total score for a given user: the sum of the score for all their scorecards.
     *
     ...
     */
    @Query("""
            SELECT NEW microservices.book.gamification.game.domain.LeaderBoardRow(s.userId, SUM(s.score))
            FROM ScoreCard s
